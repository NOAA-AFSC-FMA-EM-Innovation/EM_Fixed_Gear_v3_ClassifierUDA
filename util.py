...
level_2_name_to_level_1_name = {}
for level_1_name in hierarchy_dict:
    level_2_names = hierarchy_dict[level_1_name]

    for level_2_name in level_2_names:
        level_2_name_to_level_1_name[level_2_name] = level_1_name
...
level_2_names_to_id = {}
for level2_id, name in enumerate(level_2_names):
    level_2_names_to_id[name] = level2_id

level_2_names_to_2_id ={}
for level1_id, key in enumerate(list(hierarchy_dict.keys())):
    for species_name in hierarchy_dict[key]:
        level_2_names_to_2_id[species_name] = [level1_id, level_2_names_to_id[species_name]]


level_2_names_to_2_id_split ={}
for level1_id, key in enumerate(list(hierarchy_dict.keys())):
    for level2_id, species_name in enumerate(hierarchy_dict[key]):
        level_2_names_to_2_id_split[species_name] = [level1_id, level2_id]
...
def put_in_dict_fast_model7_level2_only(tp1, num_examples2, tp2_p1p2_31, probas_level2,targets_31, targets_split):
    num_level_1 = len(tp1)
    ...
    for level_1_id in range(num_level_1):  #遍历每一个level-1的 类别，例如 level_1_id = 0
        ...
        # num_current_level_2 = probas_level_2.size(1)    #Skates 下有2个类别
        num_current_level_2 = len(hierarchy_dict[level_1_names[level_1_id]])
    ...
    return num_examples2, tp2_p1p2_31
...
def avg_acc(num_examples, tp, acc):
    ...
    for name in num_examples:
        if num_examples[name]==0:
            print('avg_acc: %s not in eval dataset, continue: '%name)
            acc[name]=0
            continue
...
def avg_acc_track_based(num_examples, tp, acc):
    ...
    for name in num_examples:
        ...
        nominator += tp[name]
        if num_examples[name]==0:
            print('avg_acc_track_based: %s not in eval dataset: '%name)
            acc[name] = 0
            continue
...
def get_2_index_level2(header):

    head_idx, column_idx = level_2_names_to_2_id[header]
...
def choose_probs_level2_only(probas, header):

    head_idx, column_idx = get_2_index_level2(header)
    data = probas[:,column_idx]


    return data


def choose_data_level2_only(header,probas, img_names, targets_31,ids):
    ...
    if header=='id':
        ...
        data = choose_probs_level2_only(probas, header)
        data = data.tolist()
...
def per_img_predictions_write_to_csv_level2_only(i, epoch,probas_level2, img_names, save_path,targets_31, ids):

    if not os.path.exists(save_path):
        os.makedirs(save_path)
    csv_path = os.path.join(save_path,'epoch_'+str(epoch)+".csv")

    headers = ['id','img_name','level-1 gt', 'level-2 gt'] +level_2_names # only flat classifier, 34 species
    all_data = {}
    for header in headers:
        all_data[header] = choose_data_level2_only(header,probas_level2, img_names, targets_31,ids)
...
def compute_accuracy_model7_track_based_level_2_only(model, data_loader, epoch,device, save_path, lmmd=False, multi_level=False):
    # 初始化 统计字典
    tp1 = {}
    num_examples1 = {}
    acc_1 = {}
    ...
    for i, (imgs, targets_all, targets_split, img_names, ids) in tqdm(enumerate(data_loader)):
        imgs = imgs.to(device)
        targets_31 = targets_all.to(device)
        targets_split = targets_split.to(device)
        ...
        if lmmd:
            ...
            if multi_level:
                probas_level2 = clf(pooling(features[-1]).squeeze())
            else:
                probas_level2 = clf(features)
        else:
            probas_level2= model(imgs)  ### model 67

        m = torch.nn.Softmax(dim=1)
        probas_level2 = m(probas_level2)




        #写入csv文件
        per_img_predictions_write_to_csv_level2_only(i,epoch, probas_level2, img_names,save_path, targets_31, ids)
        ...
        num_examples2, tp2_p1p2_31 = put_in_dict_fast_model7_level2_only(tp1,
                                                                       num_examples2,
                                                                       tp2_p1p2_31,
                                                                       probas_level2,
                                                                       targets_31,
    ...
    return  avg_level_2_acc_p1p2_31, acc_2_p1p2_31
...
def read_cls_csv_as_dict(area_order_file):
    ...
    for i, row in enumerate(dict_reader_all):
        ...
        species, found = alias_func.convertName(row['species'])

        if species not in level_2_names:
            print('not in alias: ', species)
            continue
        ...
        filtered_row['class'] = species
        filtered_row['conf'] = 1.0
        filtered_row['length'] = 0
        filtered_row['kept'] = -2

        track_target[track_id].append(filtered_row)
    csvFile_all.close()
...
def track_based_accuracy(save_path, epoch, threshold_1):
    ...
    track_predict_level_2_names = [] # used for confusion matrix
    track_gt_level_2_names = []
    tp_1={}
    total_num_1 = {}
    tp_2 = {}
    total_num_2 = {}
    ...
    for j, track_id in enumerate(valid_track,1):
        ...
        if level_2_gt!='-1': #只有level-2有 gt的，才进行level-2的准确率计算
            ...
            track_gt_level_2_names.append(name_2_gt)
            ...
            track_predict_level_2_names.append(name_2_pred)
    ...
    for name in level_2_names:
        if name =='Shortraker-Rougheye-BlackSpotted Rockfish':
            labels_names.append('SRB')
        else:
            labels_names.append(name)


    cfm = confusion_matrix(track_gt_level_2_names, track_predict_level_2_names, labels=labels_names)
    df_cm = pd.DataFrame(cfm, index=labels_names,
                         columns=labels_names)

    plt.figure(figsize=(10, 7))
    sn.heatmap(df_cm, annot=True, cmap=sn.cubehelix_palette(as_cmap=True))
    plt.savefig('avg_conf_confusion_level_2.png',bbox_inches='tight')
    plt.show()
...
def track_based_accuracy_level2_only(save_path, epoch):
    ...
    return avg_level_2_acc_31, acc_2_31
...
def track_based_accuracy_majority_vote(save_path, epoch):
    ...
    track_gt_level_2_names, track_predict_level_2_names = [],[]
    ...
    plt.figure(figsize=(10, 7))
    sn.heatmap(df_cm, annot=True, cmap=sn.cubehelix_palette(as_cmap=True))
    plt.savefig('majority_vote_confusion_level_2.png',bbox_inches='tight')
    plt.show()
